...
static Buffer draw_char(FT_Face face, char c) {
    ...
    for (unsigned int y = 0; y < bitmap.rows; y++) {
        ...
        for (unsigned int byte_index = 0; byte_index < (unsigned int) bitmap.pitch; byte_index++) {
            ...
            //std::cout << "  i: " << byte_index << ", " << std::bitset<8>(byte_value) << std::endl;
            ...
            uint8_t max_index = std::min(8u, bitmap.width - num_bits_done);
            for (uint8_t bit_index = 0; bit_index < max_index; bit_index++) {
