...
static const char* chars_to_render = "0123456789:";
static const size_t chars_to_render_nb = 11;
...
static Image draw_char(FT_Face face, char c, int outline_width) {
    ...
    if (error) {
        std::cout << "Error digit FT_Stroker_New" << std::endl;
    }
    ...
    if (error) {
        std::cout << "Error digit FT_Get_Glyph" << std::endl;
    }
    error = FT_Glyph_Stroke(&glyph_outline, stroker, 1);
    if (error) {
        std::cout << "Error digit FT_Glyph_Stroke: " << error << std::endl;
    }
    error = FT_Glyph_To_Bitmap(&glyph_outline, FT_RENDER_MODE_MONO, NULL, true);
    if (error) {
        std::cout << "Error digit FT_Glyph_To_Bitmap" << std::endl;
    }
    ...
    std::function<void(const GeneratedTexture&, const Glyph*)> callback)
{
    ...
    if (error) {
        std::cout << "Error digit FT_Set_Pixel_Sizes" << std::endl;
    }
