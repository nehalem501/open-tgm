...
void load_texture(Texture& texture, const uint8_t* img_data, const size_t img_data_size) {
    uint8_t* gpu_mem = (uint8_t*) linearAlloc(img_data_size);

    // Copy data to GPU
    memcpy(gpu_mem, img_data, img_data_size);

    // Ensure data is in physical ram
    GSPGPU_FlushDataCache(gpu_mem, img_data_size);

    // Load the texture and bind it to the first texture unit
    C3D_TexInit(&texture.handle, texture.width, texture.height, GPU_RGBA8); // TODO texture format
